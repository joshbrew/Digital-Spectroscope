...
export const CanvasToBMP = {
    ...
    toArrayBuffer: function(imagedata) {
      ...
      var w = canvas.width,
          h = canvas.height,
          w4 = w * 4,
          data32 = new Uint32Array(imagedata.data.buffer), // 32-bit representation of canvas
    ...
    toBlob: function(canvas) {
      return new Blob([this.toArrayBuffer(this.getImageData(canvas))], {
        type: "image/bmp"
      });
    },
    ...
    toDataURL: function(canvas) {
      var buffer = new Uint8Array(this.toArrayBuffer(this.getImageData(canvas))),
          bs = "", i = 0, l = buffer.length;
      while (i < l) bs += String.fromCharCode(buffer[i++]);
      return "data:image/bmp;base64," + Buffer.from(bs, 'base64').toString('ascii');
    },

    ImageDatatoDataURL: function(imagedata) {
      var buffer = new Uint8Array(this.toArrayBuffer(imagedata)),
          bs = "", i = 0, l = buffer.length;
      while (i < l) bs += String.fromCharCode(buffer[i++]);
      return "data:image/bmp;base64," + Buffer.from(bs, 'base64').toString('ascii');
    }
